...
const char *Scorer_Combined::c_param_weight[SC_NUM] =
{
	...
	"long_weight_predtor",		// SC_PREDTOR
...
const char *Scorer_Combined::c_param_short_weight[SC_NUM] =
{
	...
	"short_weight_predtor",			// SC_PREDTOR
		...
		if (name == c_param_filename[n])
		{
			switch (n)
			{
				...
				case SC_TOR:
					...
					break;
                                case SC_PREDTOR:
                                        m_predtor->set_long_data_file(value);
                                        break;
				case SC_SAULO:
		...
		if (name == c_param_short_filename[n])
		{
			switch (n)
			{
				...
				case SC_TOR:
					...
					break;
                                case SC_PREDTOR:
                                        m_predtor->set_short_data_file(value);
                                        break;
				case SC_SAULO:
		...
		if (w != 0.0 )
		{
			switch (n)
			{
				...
				case SC_TOR:	s = m_torsion->score(p, vbose); break; 
                                case SC_PREDTOR:s = m_predtor->score(p, vbose); break;
			...
			/* These scores are part of the CORE component, so they should not be added to "total" */
			if(n != SC_RAPDF && n != SC_LJ)
				total += s;
		}
	}
